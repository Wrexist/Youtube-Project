...
| Repurpose | live for reads (`GET /v1/repurpose/clips`) and for recording a grant (`POST .../grant`), falling back to `demo.ts`. "Build episode" is still `disabled` and carries the reason — the `repurpose` workflow runs end to end from `POST /v1/jobs` and produces a publishable file, but the screen has no timeline editor, so there is nothing for the button to send. The originality report on the screen is **always the fixture**, labelled "example report": `POST /v1/repurpose/evaluate` is live and typed, and the workflow's `originality` stage produces a real one on every run, but the screen cannot yet describe a timeline to ask about |
