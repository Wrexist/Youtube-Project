...
| Repurpose | live for reads (`GET /v1/repurpose/clips`) and for recording a grant (`POST .../grant`), falling back to `demo.ts`. "Build episode" is still `disabled` and carries the reason — the `repurpose` workflow runs end to end from `POST /v1/jobs`, but the screen has no timeline editor to assemble one from, so there is nothing to send. The originality report on the screen is **always the fixture**, labelled "example report": `POST /v1/repurpose/evaluate` is live and typed, and the workflow's own `originality` stage produces a real one, but the screen cannot yet build the timeline to ask about |
